...
class AuthChecker {
  ...
  Future<String> _encryptString(String s) async {
  ...
  Future<MemoModelUser?> createUserFromMnemonic() async {
    ...
    String? encryptedWif = prefs.getString("encrypted_wif");

    try {
      return MemoModelUser.fromBackup(mnemonic: await _decryptMnemonic(encryptedMnemonic), wif: await _decryptMnemonic(encryptedWif));
    } catch (e) {
      Sentry.captureException(e);
      Sentry.logger.error("Error decrypting mnemonic or wif: $e");
      return null;
    }
  }
  ...
  Future<String> loginInWithMnemonic({String? mnemonic, String? wif}) async {
    try {
      ...
      if (mnemonic != null) {
        ...
        String encryptedMnemonic = await _encryptString(mnemonic);
        final prefs = _ref.read(sharedPreferencesProvider);
        await prefs.setString("encrypted_mnemonic", encryptedMnemonic);
      } else if (wif != null) {
        if (MemoVerifier(wif).isValidWif()) {
          String encryptedWif = await _encryptString(wif);
          final prefs = _ref.read(sharedPreferencesProvider);
          await prefs.setString("encrypted_wif", encryptedWif);
        } else
          return "invalid wif";
      }
  ...
  Future<String> logOut() async {
    try {
      ...
      await prefs.remove("encrypted_wif");

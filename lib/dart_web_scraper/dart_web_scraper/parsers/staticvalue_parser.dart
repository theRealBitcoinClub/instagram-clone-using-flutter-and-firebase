...
Data? staticValueParser({required Parser parser, required Data parentData, required bool debug}) {
  printLog("----------------------------------", debug, color: LogColor.yellow);
  printLog("ID: ${parser.id} Parser: Static Value", debug, color: LogColor.cyan);

  // Return static string value if configured
  if (parser.parserOptions?.staticValue?.stringValue != null) {
    return Data(parentData.url, parser.parserOptions!.staticValue!.stringValue!);
  }
  ...
  // No static value configured
    ...
    printLog("Static Value Parser: No data found!", debug, color: LogColor.red);
    return null;
  }
}

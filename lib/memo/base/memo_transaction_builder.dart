...
class MemoTransactionBuilder implements BasedBitcoinTransacationBuilder {
  ...
  static int estimateTransactionSize({
    ...
    final transaction = transactionBuilder.buildTransaction((trDigest, utxo, multiSigPublicKey, int sighash) {
  ...
  Script _buildInputScriptPubKeys(UtxoWithAddress utxo) {
    ...
    switch (utxo.utxo.scriptType) {
      ...
      default:
        throw DartBitcoinPluginException('${utxo.utxo.scriptType} does not sudpport on ${network.conf.coinName.name}');
    }
  }
...
*/
  List<String> _buildUnlockingScript(String signedDigest, UtxoWithAddress utx) {
    ...
    switch (utx.utxo.scriptType) {
      ...
      default:
        throw DartBitcoinPluginException('Cannot send from this type of address ${utx.utxo.scriptType}');
    }
  }
  ...
  List<TxOutput> _buildOutputs() {
    var outputs = outPuts.where((element) => element is! BitcoinBurnableOutput).map((e) => e.toOutput).toList();

    if (memo != null && memoCode != null) {
      outputs.add(TxOutput(amount: BigInt.zero, scriptPubKey: memoOpReturn(memo!, memoCode!, memoTopic)));
    }
    ...
    for (final i in sumOfTokenUtxos.entries) {
      if (sumTokenOutputAmouts[i.key] != i.value) {
        ...
        amount += outPuts
            ...
            .fold(BigInt.zero, (previousValue, element) => previousValue + element.value);
    ...
    for (final i in utxos) {
      ...
      if (token != null && token.hasNFT) {
        ...
        final hasOneoutput = outPuts.whereType<BitcoinTokenOutput>().any(
          (element) => element.utxoHash == i.utxo.txHash && element.token.category == token.category,
        );
        if (hasOneoutput) continue;
        final hasBurnableOutput = outPuts.whereType<BitcoinBurnableOutput>().any(
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (indexUtxo.isMultiSig()) {
        ...
        for (var ownerIndex = 0; ownerIndex < multiSigAddress.signers.length; ownerIndex++) {
          ...
          final sig = sign(digest, indexUtxo, multiSigAddress.signers[ownerIndex].publicKey, sighash);
  ...
  Future<BtcTransaction> buildTransactionAsync(BitcoinSignerCallBackAsync sign) async {
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (indexUtxo.isMultiSig()) {
        ...
        for (var ownerIndex = 0; ownerIndex < multiSigAddress.signers.length; ownerIndex++) {
          /// now we need sign the transaction digest
          final sig = await sign(digest, indexUtxo, multiSigAddress.signers[ownerIndex].publicKey, sighash);

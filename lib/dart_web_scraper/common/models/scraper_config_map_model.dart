...
class ScraperConfigMap {
  ...
  ScraperConfigMap({required this.configs, this.useNth});
  ...
  factory ScraperConfigMap.fromMap(Map<String, dynamic> map) {
    ...
    final parsed = raw.map<String, List<ScraperConfig>>((key, value) {
      final list = (value as List<dynamic>).map((e) => ScraperConfig.fromMap(e as Map<String, dynamic>)).toList();
      return MapEntry(key, list);
    });

    return ScraperConfigMap(configs: parsed, useNth: map['useNth'] as int?);
  }

  Map<String, dynamic> toMap() => {
    'configs': configs.map((k, v) => MapEntry(k, v.map((c) => c.toMap()).toList())),
    'useNth': useNth,
  };

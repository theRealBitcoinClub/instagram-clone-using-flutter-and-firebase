...
class PostService {
  ...
  static const String collectionName = FirestoreCollections.posts;
  ...
  Future<List<MemoModelPost>> getPostsPaginated({
    ...
    Query query = _firestore.collection(collectionName).orderBy(orderByField, descending: descendingOrder);
  ...
  Future<int> getTotalPostCount() async {
    try {
      final querySnapshot = await FirebaseFirestore.instance.collection(collectionName).count().get();

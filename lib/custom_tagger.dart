...
///
/// [CustomFlutterTagger] maintains tag positions during text editing and allows
...
class CustomFlutterTagger extends StatefulWidget {
  ...
  /// whenever [CustomFlutterTagger] enters the search context.
  /// {@endtemplate}
  final FlutterTaggerSearchCallback onSearch;

  /// {@template builder}
  /// Widget builder for [CustomFlutterTagger]'s associated TextField.
  ///  {@endtemplate}
...
class _CustomFlutterTaggerState extends State<CustomFlutterTagger> {
  ...
  void _animationControllerListener() {
    if (widget.animationController?.status == AnimationStatus.dismissed && _overlayController.isShowing) {
      _overlayController.hide();
    }
  }
  ...
  /// of calling [CustomFlutterTagger.tagTextFormatter] if it's not null.
  ...
  /// and calls [CustomFlutterTagger.onSearch].
  ...
  Widget build(BuildContext context) {
    return OverlayPortal(
      ...
      overlayChildBuilder: (context) {
        ...
        return Positioned(left: offset.dx, width: width, height: widget.overlayHeight, top: top, bottom: bottom, child: widget.overlay);
      },
    );
  }
}
...
class CustomFlutterTaggerController extends TextEditingController {
  ...
  /// Formatted text from [CustomFlutterTagger]
  ...
  /// Specify this if you supply your own [CustomFlutterTagger.tagTextFormatter].
  ///
  ...
  /// Defers [CustomFlutterTagger]'s listener attached to this controller.
  void _runDeferedAction(Function action) {
    _deferCallback?.call();
    action.call();
  }

  /// Clears [CustomFlutterTagger] internal tag state.
  @override
  void clear() {
    _clearCallback?.call();
    _text = '';
  ...
  /// Registers callback for clearing [CustomFlutterTagger]'s
  /// internal tags state.
  void _onClear(Function callback) {
    _clearCallback = callback;
  }

  /// Registers callback for dismissing [CustomFlutterTagger]'s overlay.
  void _onDismissOverlay(Function callback) {
    _dismissOverlayCallback = callback;
  }

  /// Updates [_text] with updated and formatted text from [CustomFlutterTagger].
  ...
  void _registerFormatTagTextCallback(String Function(String id, String tag, String triggerCharacter) callback) {

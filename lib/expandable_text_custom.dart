...
class ExpandableTextCustom extends StatefulWidget {
  const ExpandableTextCustom(
    this.text, {
    Key? key,
  ...
  @override
  ExpandableTextStateCustom createState() => ExpandableTextStateCustom();
}

class ExpandableTextStateCustom extends State<ExpandableTextCustom> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(ExpandableTextCustom oldWidget) {
        ...
        if (linkText.length > 0)
          TextSpan(
            ...
            children: <TextSpan>[
              ...
              TextSpan(text: linkText, style: linkTextStyle, recognizer: _linkTapGestureRecognizer),
            ],
          ),
      ],
    );

    final prefix = TextSpan(text: prefixText, style: effectiveTextStyle.merge(widget.prefixStyle), recognizer: _prefixTapGestureRecognizer);

    final text = _textSegments.isNotEmpty
        ? TextSpan(children: _buildTextSpans(_textSegments, effectiveTextStyle, null))
        : TextSpan(text: widget.text);

    final content = TextSpan(children: <TextSpan>[prefix, text], style: effectiveTextStyle);
      ...
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        if (textPainter.didExceedMaxLines) {
          ...
          final text = _textSegments.isNotEmpty
              ...
              : TextSpan(text: _expanded ? widget.text : widget.text.substring(0, max(endOffset, 0)), recognizer: recognizer);

          textSpan = TextSpan(style: effectiveTextStyle, children: <TextSpan>[prefix, text, link]);
        } else {
          textSpan = content;
        }
  ...
  void _updateText() {
    ...
    _textSegments.forEach((element) {
      if (element.isUrl && widget.onUrlTap != null) {
        ...
        _textSegmentsTapGestureRecognizers.add(recognizer);
      } else if (element.isTopic && widget.onMentionTap != null) {
        final recognizer = TapGestureRecognizer()
          ..onTap = () {
  ...
  List<TextSpan> _buildTextSpans(List<TextSegment> segments, TextStyle textStyle, TapGestureRecognizer? textTapRecognizer) {
    ...
    for (var segment in segments) {
      ...
      } else if (segment.isTopic && widget.onMentionTap != null) {
      ...
      final span = TextSpan(text: segment.text, style: style, recognizer: recognizer ?? textTapRecognizer);
...
class TextSegment {
  ...
  final bool isTopic; // Changed from isMention to isTopic
  final bool isUrl;

  bool get isText => !isHashtag && !isTopic && !isUrl;

  TextSegment(
    this.text, [
    this.name,
    this.isHashtag = false,
    this.isTopic = false, // Changed from isMention to isTopic
    this.isUrl = false,
  ]);
  ...
  bool operator ==(Object other) =>
      ...
      other is TextSegment &&
          ...
          isTopic == other.isTopic && // Changed from isMention to isTopic
          isUrl == other.isUrl;
  ...
  int get hashCode =>
      ...
      name.hashCode ^
      isHashtag.hashCode ^
      isTopic.hashCode ^ // Changed from isMention to isTopic
      isUrl.hashCode;
}

/// Split the string into multiple instances of [TextSegment] for topics, hashtags, URLs and regular text.
///
/// Topics are all words that start with @, e.g. @topic.
...
List<TextSegment> parseText(String? text) {
  ...
  final RegExp exp = RegExp(combinedPattern);
  ...
  for (final match in matches) {
    ...
    if (topicRegex.hasMatch(matchedText)) {
      segments.add(
        TextSegment(
          matchedText,
          matchedText.substring(1), // Remove @ symbol for name
          false,
          true, // isTopic
          false,
        ),
  ...
  // text after the last match or the whole text if it does not contain any matches
